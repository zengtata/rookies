...
// Note: questions index 6 is the multi-select question.
const questions = [
...
// Define a type that can hold either a single string (for single-select)
// or a string array (for multi-select at question index 6)
interface Answers {
...
const Page = () => {
  ...
  const handleAnswer = (questionIndex: number, answer: string) => {
    setAnswers((prev) => {
      // For multi-select question at index 6, store as an array.
      if (questionIndex === 6) {
  ...
  const handleSubmit = async () => {
    ...
    // Build responses in order. For multi-select, join the array into a comma-separated string.
