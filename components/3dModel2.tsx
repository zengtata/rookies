...
function Model(props: { modelPath: string }) {
    ...
    // Override the bubble mesh's material
    scene.traverse((child: any) => {
        console.log(child.name);
        if (child.isMesh && (child.name === "Marble" || child.name === "Marble_Marble_0")) {
            child.material = new MeshPhysicalMaterial({
                transmission: 1, // Light passes through like glass
                transparent: true,
                opacity: 0.5,
                roughness: 0,
                metalness: 0,
                reflectivity: 0.1,
                thickness: 0.0001,  // Adjust for more or less "bubble" thickness
                envMapIntensity: 1.0,
            });
        }
    });

    // Default transformation values (adjust to your preference)
    const rotation = [0, -3, 0]; // initial rotation offset
    const scale = 0.03;
    const position = [0, 0, 0];

    // Rotate the group automatically
    useFrame((state, delta) => {
        if (groupRef.current) {
            groupRef.current.rotation.y += delta * 0.5; // adjust speed as desired
        }
    });
